...
def send_process_query(query):
    ...
    setupTempaltePromt = PromptTemplate.from_template(
        "You are an AI that send  professional email. The email belongs to 'parthakadam2007@gmail' and the name is Partha Kadam. write mail  professionally and provide all requested details , email:{email}, context:{context}"
    )
